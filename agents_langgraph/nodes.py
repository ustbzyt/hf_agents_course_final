...
def assistant(state: AgentState) -> AgentState:
    # 1. 初始化消息列表，添加 system prompt
    if not state.get('messages'):
        tools_desc = (
            "get_weather_info(location: str) -> str: Fetches real-time weather information for a given location using OpenWeatherMap API.\n"
            "retrieve_guest_info(query: str) -> str: Retrieves detailed information about gala guests based on their name or relation.\n"
            ...
            "arxiv_search(query: str) -> str: Searches arXiv for academic papers and summaries.\n"
    ...
    if state.get("question"):
        print(f"[DEBUG] User question: {state['question']}")
    if state["messages"]:
    ...
    time.sleep(3)  # 每次 LLM 推理后等待3秒，防止超时和限流
    ...
    # 5. 标记消息类型，并提取 FINAL ANSWER
    if isinstance(result, AIMessage):
        if "final answer" in result.content.lower():
            result.type = "final"
            # 提取 FINAL ANSWER 后的内容，赋值给 state["final_answer"]
